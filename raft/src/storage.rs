use crate::{LogIndex, Term, StateMachine};
use std::collections::BTreeMap;

pub(crate) trait Storage<S: StateMachine> {
    ...
    fn append(&mut self, entries: Vec<S::Command>, start_index: LogIndex, term: Term);
...
impl<S: StateMachine> Storage<S> for MemoryStorage<S> {
    fn has_entry(&self, log_index: LogIndex, log_term: Term) -> bool {
    ...
    fn append(&mut self, entries: Vec<S::Command>, start_index: LogIndex, term: Term) {
        if start_index > 1 && self.last_log_index() < (start_index - 1) {
            panic!("MemoryStorage::append called with start_index {} and last_log_index {}",
